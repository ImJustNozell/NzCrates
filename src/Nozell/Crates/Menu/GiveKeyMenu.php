...
final class GiveKeyMenu extends CustomForm
{
    ...
    public function __construct(Player $player)
    {
        ...
        $this->setTitle(LangManager::getInstance()->generateMsg('form-title-give-key', [], []));
        $this->addDropdown(LangManager::getInstance()->generateMsg('form-dropdown-key-type', [], []), $this->keyTypes);
        $this->addInput(LangManager::getInstance()->generateMsg('form-input-amount', [], []), LangManager::getInstance()->generateMsg('form-input-amount-placeholder', [], []));
        $this->addDropdown(LangManager::getInstance()->generateMsg('form-dropdown-player', [], []), $this->onlinePlayers);
    ...
    public function handleResponse(Player $player, $data): void
    {
        ...
        if ($targetPlayer === null) {
            $player->sendMessage(LangManager::getInstance()->generateMsg('player-not-online', [], []));
            return;
        }
        ...
        $msg = LangManager::getInstance()->generateMsg('given-keys-to-player', ['{amount}', '{keyType}', '{playerName}'], [$amount, $keyType, $targetPlayer->getName()]);
